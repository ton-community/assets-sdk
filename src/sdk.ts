...
export class AssetsSDK {
    ...
    async deployJetton(content: JettonContent, options?: DeployJettonOptions) {
        ...
        if (adminAddress === undefined) {
            throw new Error(
                "Admin address must be defined in options or be available in Sender"
            );
        }
        ...
        const jettonMinterContract = JettonMinter.createFromConfig(
            {
                admin: adminAddress,
                content: await this.contentToCell(
                    jettonContentToInternal(content),
    ...
    openJetton(address: Address) {
        return this.api.open(
            JettonMinter.createFromAddress(address, this.contentResolver)
        );
    }
    ...
    async deployNftCollection(
            ...
            NftCollection.createFromConfig(
                {
                    ...
                    content: beginCell()
                        ...
                        .storeRef(
                            beginCell().storeStringTail(content.commonContent)
                        )
                        .endCell(),
        ...
        if (
            typeof options?.premintItems?.length === "number" &&
            options?.premintItems.length > 0
        ) {
    ...
    openNftCollection(address: Address) {
        return this.api.open(
            NftCollection.createFromAddress(address, this.contentResolver)
        );
    }
    ...
    openSbtCollection(address: Address) {
        return this.api.open(
            SbtCollection.createFromAddress(address, this.contentResolver)
        );
    }
    ...
    openNftItem(address: Address) {
        return this.api.open(
            new NftItem(address, undefined, this.contentResolver)
        );
    }
    ...
    async deployNftSale(params: NftSaleParams) {
        ...
        if (marketplaceAddress === undefined) {
            throw new Error(
                "Marketplace address must be defined in options or be available in Sender"
            );
        }
        ...
        await sale.sendTopup(this.sender, {
            value: params.value,
    ...
    private async internalOffchainContentToCell(
        internal: Record<string, string | string[] | number | undefined>
    ) {
        const contentUrl = await this.storage.uploadFile(
            Buffer.from(JSON.stringify(internal), "utf-8")
        );
        return beginCell()
            .storeUint(0x01, 8)
    ...
    private async contentToCell(
        internal: Record<string, string | string[] | number | undefined>,
        onchain: boolean
    ) {
        return onchain
            ? internalOnchainContentToCell(internal)
            : await this.internalOffchainContentToCell(internal);
    }
}
